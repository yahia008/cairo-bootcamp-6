...
pub trait IAdminRestricted<TContractState> {
    ...
    fn admin_revoke_spender(ref self: TContractState, owner: ContractAddress, spender: ContractAddress);
}
...
pub mod erc20 {
    ...
    impl IAdminRestrictedImpl of super::IAdminRestricted<ContractState> {
        fn set_transfer_limit(ref self: ContractState, new_limit: u256) {
            ...
            self.emit(TransferLimitUpdated { old_limit, new_limit, updated_by: get_caller_address() });
        }
        ...
        fn get_transfer_limit(self: @ContractState) -> u256 { self.transfer_limit.read() }
        fn is_transfers_enabled(self: @ContractState) -> bool { self.transfers_enabled.read() }
